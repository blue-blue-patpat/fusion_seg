...
# from visualization.mesh_plot import MoshEvaluateStreamPlot, pcd2box
from nn.p4t.modules.loss import LossManager


def train_one_epoch(args, model, losses, criterion, optimizer,  lr_scheduler, data_loader_train, device, epoch):
    
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")     #记录损失和参数
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value}'))
    metric_logger.add_meter('clips/s', utils.SmoothedValue(window_size=10, fmt='{value:.3f}'))  #track the performance
    header = 'Epoch: [{}]'.format(epoch)
    
    for input, label in metric_logger.log_every(data_loader_train, args.print_freq, header):
        ...
        losses.update_loss("loss", criterion(output, label))
...
# def evaluate(args, model, losses, criterion, data_loader, device, save_path=''):
#     model.eval()
...
#             criterion = nn.CrossEntropyLoss()
#             losses.update_loss("loss", criterion(output, target))
#             loss = losses.calculate_total_loss()
...
#             #         yield dict(
#             #             points_pcd = dict(
#             #                 # mesh = pcd2box(data_dict['points'][i][:,:3]) if data_dict['points'].shape[1] else None,
#             #                 pcd = c2c(data_dict['points'][i][:,:3]) if data_dict['points'].shape[1] else None,
#             #                 color = [0,0.8,0],
#             #             )
                        
#             #             # master_pcd = dict(
#             #             #     # mesh = pcd2box(data_dict['master_depth'][i][:,:3]) if data_dict['master_depth'].shape[1] else None,
#             #             #     pcd = c2c(data_dict['master_depth'][i][:,:3]) if data_dict['master_depth'].shape[1] else None,
#             #             #     color = np.asarray([59, 170, 235]) /255,
#             #             # ),
#             #             # sub_pcd = dict(
#             #             #     # mesh = pcd2box(data_dict['sub_depth'][i][:,:3]) if data_dict['sub_depth'].shape[1] else None,
#             #             #     pcd = c2c(data_dict['sub_depth'][i][:,:3]) if data_dict['sub_depth'].shape[1] else None,
...
def main(args):
    ...
    #dataset_train = torch.utils.data.random_split(dataset, [train_size])
    ...
    #la_criterion = nn.CrossEntropyLoss(args.output, args.label)
    ...
    criterion = dict(mse=mse_criterion, rot_mat=rot_mat_criterion, entropy=entropy_criterion)
    ...
    if args.train:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            train_one_epoch(args, model, losses, criterion,  optimizer, lr_scheduler, data_loader_train, device, epoch)
            losses.calculate_epoch_loss(os.path.join(output_dir, "loss/train"), epoch)
            #list(evaluate(args, model, losses, criterion, data_loader_eval, device))
        ...
        gen = evaluate(args, model, losses,pred, criterion, data_loader_test, device, save_path)
...
def parse_args():
    ...
    # input
    parser.add_argument('--data_path', default='/home/nesc525/drivers/4/gjn/SegmentationDataset', type=str, help='dataset')
    parser.add_argument("--seq_idxes", type=str, default='2') 
    ...
    parser.add_argument('--features', default=0, type=int, help='dim of features')
    parser.add_argument('--dataset', default='points,images', type=str, help='type of input data, mmWave, Depth or mmFusion')
    parser.add_argument('--test_scene', default="test", type=str, help='type of test data, test, rain, smoke, night, occlusion, confusion')
    #parser.add_argument('--inputs', type=str, default='points,images,master_depth,sub_image,sub_depth', help='input data')
    parser.add_argument('--inputs', type=str, default='points,images', help='input data')
    ...
    parser.add_argument('--output_dir', default='/home/nesc525/drivers/4/gjn', type=str, help='path where to save')
    # resume
    parser.add_argument('--resume', default='', help='resume from checkpoint')
    parser.add_argument('--start_epoch', default=0, type=int, metavar='N', help='start epoch')
    parser.add_argument('--train', dest="train", default='True', action="store_true", help='train or test')
    ...
    parser.add_argument('--read_orig_img', default='True', action="store_true")
