...
from tools.eval import seg_eval
...
def train_one_epoch(args, model, losses, criterions, loss_weight, optimizer, lr_scheduler, data_loader, device, epoch):
    ...
    for input in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        img_seg_logit = torch.flatten(img_seg_logit, start_dim=2)
        img_seg_label = torch.flatten(img_seg_label, start_dim=1)
...
def evaluate(args, model, losses, criterion, data_loader, device, save_path=''):
    ...
    img_miou = []
    pcd_miou = []
    label2cat = {0:'backgroud', 1:'smooth', 2:'rough', 3:'bumpy', 4:'forbidden', 5:'obstacle'}
    with torch.no_grad():
        for input in metric_logger.log_every(data_loader, args.print_freq, header):
            label = ['img_label', 'pcd_label']
            if isinstance(input, dict):
                for k, v in input.items():
                    if k in label:
                        input[k] = v.to(device, non_blocking=True, dtype=torch.int64)
                    else:
                        if isinstance(v, torch.Tensor):
            ...
            img_seg_label, pcd_seg_label = input['img_label'], input['pcd_label']
            _, pcd_seg_pred = pcd_seg_logit.topk(1, dim=1)
            pcd_seg_pred = pcd_seg_pred.squeeze(1)
            img_miou.append(seg_eval(img_seg_label, img_seg_pred, label2cat))
            pcd_miou.append(seg_eval(pcd_seg_label, pcd_seg_pred, label2cat))
        ...
        if save_path:
            ...
            img_error = c2c(torch.vstack(img_miou))
            pcd_error = c2c(torch.vstack(pcd_miou))
            np.save(os.path.join(save_path, "img_result"), img_error)
            np.save(os.path.join(save_path, "pcd_result"), pcd_error)
