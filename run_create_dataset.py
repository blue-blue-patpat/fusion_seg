...
from nn.datasets.utils import crop_image, filter_pcl
...
def make_dirs(save_path: str):
    ...
    if not os.path.exists(os.path.join(save_path, 'bouding_box', 'master')):
        os.makedirs(os.path.join(save_path, 'bouding_box', 'master'))
    if not os.path.exists(os.path.join(save_path, 'bouding_box', 'sub')):
        os.makedirs(os.path.join(save_path, 'bouding_box', 'sub'))
...
def save_data(save_path: str, seq_loader: ResultFileLoader):
    ...
    for idx in range(len(seq_loader)):
        ...
        crop_img, mas_box_min, mas_box_max = crop_image(mesh_joint, master_img, trans_mat=trans_mat['kinect_master'], square=True, return_box=True)
        ...
        crop_img, sub_box_min, sub_box_max = crop_image(mesh_joint, sub2_img, trans_mat=trans_mat['kinect_sub'], square=True, cam="sub", return_box=True)
        ...
        np.save(os.path.join(save_path, 'bouding_box', 'master', 'frame_{}'.format(idx)), master_bbox)
        np.save(os.path.join(save_path, 'bouding_box', 'sub', 'frame_{}'.format(idx)), sub2_bbox)
