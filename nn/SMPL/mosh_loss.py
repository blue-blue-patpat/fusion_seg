...
class SMPLXModel(BodyModel):
    ...
    def forward(self, trans, pose_body, betas, pose2rot=False):
        ...
        verts, joints = lbs(betas=shape_components, pose=full_pose, v_template=v_template,
                        ...
                        parents=self.kintree_table[0].long(), lbs_weights=self.weights, pose2rot=pose2rot)

        joints = joints + trans.unsqueeze(dim=1)
        verts = verts + trans.unsqueeze(dim=1)
        return dict(verts=verts, joints=joints)
...
class MoshLoss(_Loss):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, use_gender: int = 0, train: bool = True) -> torch.Tensor:
        ...
        per_joint_err = torch.mean(torch.norm((input_joints - target_joints), dim=2), dim=0)
        per_vertex_err = torch.mean(torch.norm((input_verts - target_verts), dim=2), dim=0)
