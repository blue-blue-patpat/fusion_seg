...
class DepthMesh3D(Dataset):
    ...
    def init_index_map(self):
        ...
        for idx, path in enumerate(videos_path):
            # init result loader
            video_loader = ResultFileLoader(root_path=path, enabled_sources=["sub1", "kinect_color", "kinect_pcl", "optitrack"])
            # add video to list
            self.video_loaders.append(video_loader)
            # remove deduplicated arbe frames and head frames
            #kinect_ids = sorted(map(int, video_loader.a_loader.file_dict["kinect_pcl"].drop_duplicates(subset=["dt"],keep="first")["id"]))[self.skip_head:-self.skip_tail-1]
            kinect_ids = sorted(map(int, video_loader.a_loader.file_dict["kinect_pcl"]["id"]))[self.skip_head:-self.skip_tail-1]
            # add valid ids to list
            self.id_list.append(kinect_ids)
            # init index_map
            for i, id in enumerate(kinect_ids):
    ...
    def get_data(self, video_loader, id):
        ...
        kinect_pcl = frame["kinect_pcl"]
        kinect_color = frame["kinect_color"]
        ...
        kinect_data = pcl_filter(label, kinect_data, 0.2)
        if kinect_data.shape[0] < 50:
            # remove bad frame
            arbe_data = None
        else:
            # normalization
            kinect_data[:,:3] = (kinect_data[:,:3] - center)/self.normal_scale
            #RGB normalization?
